 ...
 *
 * The floors are read from `budgets/coverage-budgets.json` and compared
...
const BUDGETS_PATH = resolve(REPO_ROOT, "budgets/coverage-budgets.json");
const COVERAGE_DIR = resolve(REPO_ROOT, "coverage");
const JUNIT_PATH = resolve(REPO_ROOT, "junit.xml");
...
function printRatchetHint(totals: CoverageTotals, budgets: CoverageBudgets): void {
	...
	if (fnSlack >= RATCHET_HINT || lineSlack >= RATCHET_HINT) {
		...
		console.error(
			`check-coverage: coverage exceeds floors by ≥${RATCHET_HINT}pt — consider ratcheting budgets/coverage-budgets.json up to e.g. {functions: ${suggested.functions.toFixed(2)}, lines: ${suggested.lines.toFixed(2)}}.`,
		);
	}
}
...
function writeSummaryArtifact(totals: CoverageTotals): void {
	...
	// (e.g. scripts/report-quality-metrics.ts) can render the same
	// numbers users see in the CI log. lcov.info aggregates diverge
	// here (it counts non-executable spans), so this JSON is the source
	// of truth for the "All files" aggregate.
	try {
...
function reportResult(
	...
	if (failures.length > 0) {
		for (const f of failures) {
			console.error(
				`check-coverage: ${f.metric} coverage ${f.actual.toFixed(2)}% is below floor ${f.floor.toFixed(2)}%. Add tests to lift it, or — if you're intentionally removing coverage — document the drop and lower the floor in budgets/coverage-budgets.json.`,
			);
		}
		return testExitCode === 0 ? 1 : testExitCode;
	}
