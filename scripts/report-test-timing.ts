#!/usr/bin/env bun
/**
 * report-test-timing.ts (L5 toolkit).
 *
...
export function parseJUnit(xml: string): TimingReport {
	...
	// when bun nests describe() blocks), so it's the metric we actually care
	// about regardless of what the surrounding suite element claims.
